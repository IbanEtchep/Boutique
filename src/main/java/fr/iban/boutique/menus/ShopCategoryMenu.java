        ...
        for (int i = 0; i < configurableItem.getLore().size(); i++) {
            ...
            if (line.contains("%price_display%")) {
                if (item.getPriceModifier() >= 1) {
                    ...
                    line = line.replace("%discount_percent%", String.valueOf((int) (100 - item.getPriceModifier() * 100.0)));
                }
                line = line.replace("%price%", String.valueOf((int) (item.getPrice() * item.getPriceModifier())));
                configurableItem.getLore().set(i, line);
            }
        }
        return configurableItem.setClickCallback(e -> {
            new ConfirmMenu(player, "§2§lConfirmer", String.format("§aVoulez-vous vraiment acheter " + item.getDisplay().getName() + " §apour " + item.getPrice() + " %s ?", plugin.getConfig().getString("messages.currency-name")),
                    confirmed -> {
                        if (confirmed) {
